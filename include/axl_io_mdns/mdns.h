/* vendored from https://github.com/vovkos/mdns (a fork of mjansson/mdns).
 * the fork changes the API -- mdns_record_callback_fn drops its socket, and the
 * senders/receivers are split into build/parse halves -- so it must not be mixed
 * up with an upstream mdns.h; keep this copy in sync with the fork by hand
 */
...
/* MDNS_NO_IO -- compile out everything that touches a socket, leaving the packet
 * parsers and builders for a caller that does its own I/O.
 *
 * Each *_send has a *_build half that fills the buffer and returns its size; each
 * *_recv a *_parse half that takes an already received datagram. MDNS_NO_IO also
 * drops the leading sock/from/addrlen from mdns_record_callback_fn and the parse
 * functions -- the parser only forwarded them; use user_data instead.
 *
 * Without MDNS_NO_IO nothing changes: same signatures, mdns.c builds untouched.
 */
...
//! mdns_listen_parse expects incoming query requests, mdns_discovery_parse a reply to a DNS-SD
//! service enumeration, and mdns_query_parse a reply to a mDNS query (optionally filtered to
//! only_query_id; pass 0 to accept any).
static inline size_t
mdns_listen_parse(
#ifndef MDNS_NO_IO
                  int sock, const struct sockaddr* saddr, size_t addrlen,
#endif
                  const void* buffer, size_t data_size, mdns_record_callback_fn callback, void* user_data);
...
//! Build a mDNS query packet for the given service name(s) into the supplied buffer, which must be
//! 32 bit aligned. rclass carries the MDNS_UNICAST_RESPONSE (QU) bit. Returns the packet size, or
//! <0 if error.
static inline int
mdns_query_build(mdns_record_type_t type, const char* name, size_t length, void* buffer,
                 size_t capacity, uint16_t query_id, uint16_t rclass);

static inline int
mdns_multiquery_build(const mdns_query_t* query, size_t count, void* buffer, size_t capacity,
                      uint16_t query_id, uint16_t rclass);

//! Build a mDNS answer packet into the supplied buffer, which must be 32 bit aligned. Returns the
//! packet size, or <0 if error.
static inline int
mdns_answer_unicast_build(void* buffer, size_t capacity, uint16_t query_id,
                          mdns_record_type_t record_type, const char* name, size_t name_length,
                          mdns_record_t answer, const mdns_record_t* authority,
                          size_t authority_count, const mdns_record_t* additional,
                          size_t additional_count);

static inline int
mdns_answer_multicast_build(void* buffer, size_t capacity, mdns_record_t answer,
                            const mdns_record_t* authority, size_t authority_count,
                            const mdns_record_t* additional, size_t additional_count,
                            uint16_t rclass, uint32_t ttl);

static inline int
mdns_query_answer_multicast_build(void* buffer, size_t capacity, mdns_record_t answer,
                                  const mdns_record_t* authority, size_t authority_count,
                                  const mdns_record_t* additional, size_t additional_count);

static inline int
mdns_announce_multicast_build(void* buffer, size_t capacity, mdns_record_t answer,
                              const mdns_record_t* authority, size_t authority_count,
                              const mdns_record_t* additional, size_t additional_count);

static inline int
mdns_goodbye_multicast_build(void* buffer, size_t capacity, mdns_record_t answer,
                             const mdns_record_t* authority, size_t authority_count,
                             const mdns_record_t* additional, size_t additional_count);
...
//! Same, but reports in final_offset where parsing stopped, so a large TXT record can be drained
//! through a small records array across several calls (pass the remaining length each time).
static inline size_t
mdns_record_parse_txt_ex(const void* buffer, size_t size, size_t offset, size_t length,
                         mdns_record_txt_t* records, size_t capacity, size_t* final_offset);
...
mdns_records_parse(
                   ...
                   int sock, const struct sockaddr* from, size_t addrlen,
#endif
                   const void* buffer, size_t size, size_t* offset, mdns_entry_type_t type, uint16_t query_id,
		...
		if (length <= (size - (*offset))) {
			...
			if (callback &&
			    callback(
			             ...
			             sock, from, addrlen,
#endif
			             type, query_id, rtype, rclass, ttl, buffer, size,
...
// the discovery question, also used by the receivers to recognize a DNS-SD reply
		...
		if (is_answer) {
			...
			if (callback &&
			    callback(
			             ...
			             sock, saddr, addrlen,
#endif
			             MDNS_ENTRYTYPE_ANSWER, query_id, rtype, rclass, ttl,
...
mdns_listen_parse(
	...
	for (int iquestion = 0; iquestion < questions; ++iquestion) {
		...
		if (callback && callback(
		                         ...
		                         sock, saddr, addrlen,
#endif
		                         MDNS_ENTRYTYPE_QUESTION, query_id, rtype,
...
// rclass carries the MDNS_UNICAST_RESPONSE (QU) bit -- the caller knows whether it
// wants a unicast answer, so it is not inferred from the port the socket sits on
...
mdns_query_build(mdns_record_type_t type, const char* name, size_t length, void* buffer,
	...
	query.name = name;
	query.length = length;
	return mdns_multiquery_build(&query, 1, buffer, capacity, query_id, rclass);
}
...
static inline uint16_t
mdns_query_rclass(int sock) {
	// ask for a unicast response unless we are sitting on the mDNS port
	...
	return MDNS_CLASS_IN | MDNS_UNICAST_RESPONSE;
}
...
mdns_multiquery_send(int sock, const mdns_query_t* query, size_t count, void* buffer,
                     size_t capacity, uint16_t query_id) {
	int tosend = mdns_multiquery_build(query, count, buffer, capacity, query_id,
	                                   mdns_query_rclass(sock));
	if (tosend < 0)
		return -1;
...
// receive one datagram into the buffer; returns its size, or 0 if nothing arrived
// -- which the caller cannot tell apart from a socket error, as upstream never
// could either

static inline size_t
mdns_socket_recv(int sock, void* buffer, size_t capacity, struct sockaddr_in6* addr,
                 socklen_t* addrlen) {
...
mdns_discovery_recv(int sock, void* buffer, size_t capacity, mdns_record_callback_fn callback,
	...
	struct sockaddr_in6 addr;
	socklen_t addrlen;
	size_t size = mdns_socket_recv(sock, buffer, capacity, &addr, &addrlen);
	if (!size)
		return 0;

	return mdns_discovery_parse(sock, (struct sockaddr*)&addr, addrlen, buffer, size, callback,
	                            user_data);
}
...
mdns_socket_listen(int sock, void* buffer, size_t capacity, mdns_record_callback_fn callback,
	...
	return mdns_listen_parse(sock, (struct sockaddr*)&addr, addrlen, buffer, size, callback,
	                         user_data);
}
...
mdns_query_recv(int sock, void* buffer, size_t capacity, mdns_record_callback_fn callback,
	...
	return mdns_query_parse(sock, (struct sockaddr*)&addr, addrlen, buffer, size, callback,
	                        user_data, only_query_id);
}
...
mdns_query_answer_unicast(int sock, const void* address, size_t address_size, void* buffer,
                          ...
                          const mdns_record_t* authority, size_t authority_count,
                          const mdns_record_t* additional, size_t additional_count) {
	int tosend = mdns_answer_unicast_build(buffer, capacity, query_id, record_type, name,
	                                       name_length, answer, authority, authority_count,
...
mdns_query_answer_multicast_build(void* buffer, size_t capacity, mdns_record_t answer,
                                  const mdns_record_t* authority, size_t authority_count,
                                  const mdns_record_t* additional, size_t additional_count) {
	return mdns_answer_multicast_build(buffer, capacity, answer, authority, authority_count,
	                                   additional, additional_count, MDNS_CLASS_IN, 60);
}
...
mdns_goodbye_multicast_build(void* buffer, size_t capacity, mdns_record_t answer,
	...
	return mdns_answer_multicast_build(buffer, capacity, answer, authority, authority_count,
	                                   additional, additional_count, MDNS_CLASS_IN, 0);
}
...
mdns_answer_multicast_rclass_ttl(int sock, void* buffer, size_t capacity, mdns_record_t answer,
                                 ...
                                 const mdns_record_t* additional, size_t additional_count,
                                 uint16_t rclass, uint32_t ttl) {
	int tosend = mdns_answer_multicast_build(buffer, capacity, answer, authority, authority_count,
	                                         additional, additional_count, rclass, ttl);
	if (tosend < 0)
		return -1;
...
mdns_record_parse_txt(const void* buffer, size_t size, size_t offset, size_t length,
                      mdns_record_txt_t* records, size_t capacity) {
	return mdns_record_parse_txt_ex(buffer, size, offset, length, records, capacity, NULL);
}
