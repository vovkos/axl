...
public:
	...
	{
		return Ptr.m_pfn ? SetHandlerImpl (Ptr) : Clear (), NULL;
	}
	...
	handle_t
	SetHandler_u (void* pfn)
	{
		return pfn ? SetHandlerImpl (pfn) : Clear (), NULL;
	}

	handle_t
	AddHandler (jnc::TFunctionPtr Ptr)
	{
		return Ptr.m_pfn ? AddHandlerImpl (Ptr) : NULL;
	}
	...
	handle_t
	AddHandler_u (void* pfn)
	{
		return pfn ? AddHandlerImpl (pfn) : NULL;
	}
