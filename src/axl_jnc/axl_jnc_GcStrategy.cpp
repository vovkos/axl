...
{
	static GCRegistry::Add <ShadowStackGC> ShadowStack (
		"jnc-shadow-stack", 
...
Constant*
ShadowStackGC::GetFrameMap (Function& F) 
{
	...
	Type *EltTys[] = 
	{ 
		DescriptorElts[0]->getType(),
		DescriptorElts[1]->getType()
	};
	...
	Constant *GV = new GlobalVariable(
		*F.getParent(), FrameMap->getType(), true,
...
bool 
ShadowStackGC::performCustomLowering (Function &F) 
{
	...
	Instruction *StackEntry = AtEntry.CreateAlloca(ConcreteStackEntryTy, 0, "gc_frame");
	...
	Instruction *EntryMapPtr = CreateGEP(Context, AtEntry, StackEntry, 0,1,"gc_frame.map");
	...
	Instruction *EntryNextPtr = CreateGEP(Context, AtEntry, StackEntry,0,0,"gc_frame.next");
	Instruction *NewHeadVal   = CreateGEP(Context, AtEntry, StackEntry, 0, "gc_newhead");
