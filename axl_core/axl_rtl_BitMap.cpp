...
GetBit (
	...
	size_t Bit)
{
	size_t Page = Bit / AXL_PTR_BITNESS;
	if (Page >= PageCount)
		return false;

	size_t Mask = (size_t) 1 << (Bit & (AXL_PTR_BITNESS - 1));
	...
	)
{
	...
	size_t BitCount = PageCount * AXL_PTR_BITNESS;
...
{
	size_t i = From / AXL_PTR_BITNESS;
	if (i >= PageCount)
		return -1;

	const size_t* p = pMap + i;

	From -= i * AXL_PTR_BITNESS;
	...
	if (Value)
	{
		...
		if (x)
			return i * AXL_PTR_BITNESS + GetLoBitIdx (x);

		for (i++, p++; i < PageCount; i++, p++)
		{
			if (*p != 0)
				return i * AXL_PTR_BITNESS + GetLoBitIdx (*p);
		}
	}
	else
	{
...
{
	size_t PageCount = BitCount / AXL_PTR_BITNESS;
	if (BitCount & (AXL_PTR_BITNESS - 1))
		PageCount++;
	...
	)
{
	...
	return rtl::MergeBitMaps (m_Map, BitMap2.m_Map, min (PageCount, PageCount2), Op);
}
