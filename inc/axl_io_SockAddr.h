...
struct TSockAddrIp6: TSockAddr
{
	...
	ToString () const
	{
		...
		String.Format (_T("%04x"), m_Ip6 [0]);

		for (size_t i = 1; i < 8; i++)
			if (m_Ip6 [i])
				String.AppendFormat (_T("::%04x"), m_Ip6 [i]);

		String.AppendFormat (_T(".%d"), m_Port);
		return String;
	}
};
...
union TSockAddrU
{
	...
	ToString () const
	{
		...
		default:
			return rtl::CString::Format_s (_T("<address-kind=%d>"), m_Hdr.m_Kind);
		}
	}
};
