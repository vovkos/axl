...
typedef CArgStringT <tchar_t> CArgString;
...
public:
	...
	typedef obj::CClassT <CArg> CClass;
...
CreateArgBlockV (va_list va)
{
	...
	if (T::HasShadow)
	{
		...
		Package->m_Agent.Copy (pClass, Shadow.GetRefCount ());
	}
	else
	{
		T () (Package + 1, &Size, NULL, va);
		Package->m_Agent.Copy (pClass, NULL);
	}
...
{
	return CreateArgBlockV <T> (va_start_e (Unused));
}
