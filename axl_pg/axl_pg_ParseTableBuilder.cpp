...
CParseTableBuilder::Build ()
{
	...
	for (size_t i = 0; i < SymbolCount; i++)
	{
		...
		if (pNode->m_Flags & ESymbolNodeFlag_IsPragma)
		{
			if (pNode->IsNullable ())
			{
				err::SetFormatStringError (_T("'%s': pragma cannot be nullable"), pNode->m_Name);
				err::PushSrcPosError (pNode->m_SrcPos);
				return false;
			}
			...
			{
				err::SetFormatStringError (_T("'%s': pragma cannot start with 'anytoken'"), pNode->m_Name);
				err::PushSrcPosError (pNode->m_SrcPos);
				return false;
			}
		}
...
CParseTableBuilder::CalcFirstFollow ()
{
	...
	do 
	{
		...
		for (; Sequence; Sequence++)
		{
			...
			for (long j = ChildrenCount - 1; j >= 0; j--)
			{
				CGrammarNode* pChild = pNode->m_Sequence [j];
