...
CNodeMgr::Trace ()
{
	TraceNodeArray (_T("TOKENS"), &m_TokenArray);
	TraceNodeArray (_T("SYMBOLS"), &m_SymbolArray);
	TraceNodeList (_T("SEQUENCES"), m_SequenceList.GetHead ());
	TraceNodeList (_T("BEACONS"), m_BeaconList.GetHead ());
	TraceNodeList (_T("DISPATCHERS"), m_DispatcherList.GetHead ());
	TraceNodeList (_T("ACTIONS"), m_ActionList.GetHead ());
	TraceNodeList (_T("ARGUMENTS"), m_ArgumentList.GetHead ());
	TraceNodeList (_T("CONFLICTS"), m_ConflictList.GetHead ());
	TraceNodeList (_T("LOOKAHEAD DFA"), m_LaDfaList.GetHead ());
}
...
CNodeMgr::GetTokenNode (int Token)
{
	...
	if (isprint (Token))
		pNode->m_Name.Format (_T("\'%c\'"), (tchar_t) Token);
	else 
		pNode->m_Name.Format (_T("\\0%d"), Token);
...
{
	CSymbolNode* pNode = AXL_MEM_NEW (CSymbolNode);
	pNode->m_Name.Format (_T("_tmp%d"), m_TempSymbolList.GetCount () + 1);
...
CNodeMgr::CreateSequenceNode ()
{
	...
	pNode->m_Name.Format (_T("_seq%d"), m_SequenceList.GetCount () + 1);
...
CNodeMgr::CreateBeaconNode (CSymbolNode* pTarget)
{
	...
	pBeaconNode->m_Name.Format (_T("_bcn%d(%s)"), m_BeaconList.GetCount () + 1, pTarget->m_Name);
...
CNodeMgr::CreateDispatcherNode (CSymbolNode* pSymbol)
{
	...
	pDispatcherNode->m_Name.Format (_T("_dsp%d"), m_DispatcherList.GetCount () + 1);
...
CNodeMgr::CreateActionNode ()
{
	...
	pNode->m_Name.Format (_T("_act%d"), m_ActionList.GetCount () + 1);
...
CNodeMgr::CreateArgumentNode ()
{
	...
	pNode->m_Name.Format (_T("_arg%d"), m_ArgumentList.GetCount () + 1);
...
CNodeMgr::CreateConflictNode ()
{
	...
	pNode->m_Name.Format (_T("_cnf%d"), m_ConflictList.GetCount () + 1);
...
CNodeMgr::CreateLaDfaNode ()
{
	...
	pNode->m_Name.Format (_T("_dfa%d"), m_LaDfaList.GetCount () + 1);
	...
	)
{
	...
	{
		err::SetFormatStringError (_T("can't apply quantifier to action or epsilon nodes"));
		return NULL;
	}
