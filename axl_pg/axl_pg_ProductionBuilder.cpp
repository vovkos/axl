	...
	)
{
	...
	rtl::CHashTableIteratorT <const tchar_t*> It2 = m_pSymbol->m_LocalNameSet.Find (pName);
	if (It2)
	...
	err::SetFormatStringError (_T("locator '$%s' not found"), pName);
	return EVariable_Undefined;
}
	...
	)
{
	...
	for (;;)
	{
		...
		case EVariable_TokenBeacon:
			...
			{
				err::SetFormatStringError (_T("cross-resolver reference to locator '%s'"), pToken->GetText ());
				return false;
			}
			...
			ResultString.AppendFormat (_T("$%d"), pBeacon->m_SlotIndex);
			break;

		case EVariable_This:
			if (pResolver)
			{
				err::SetFormatStringError (_T("resolvers cannot reference left side of production"));
				return false;
			}
		...
		case EVariable_Arg:
			if (pResolver)
			{
				err::SetFormatStringError (_T("resolvers cannot reference arguments"));
				return false;
			}
		...
		case EVariable_Local:
			if (pResolver)
			{
				err::SetFormatStringError (_T("resolvers cannot reference locals"));
				return false;
			}
